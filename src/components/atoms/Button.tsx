...
export const Button = ({ children, variant = 'primary', fullWidth = false, className = '', disabled, ...props }: Props) => {
  const baseStyles = 'px-3 py-2 rounded transition-all duration-200';
  const variants = {
  ...
  return (
    <button className={`${baseStyles} ${variants[variant]} ${fullWidth ? 'w-full' : ''} ${className} text-base`} disabled={disabled} {...props}>
      {children}
    </button>
  );
};
